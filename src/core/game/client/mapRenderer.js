module.exports = (function(){
    ...
    MapRenderer.prototype.render= function(){
        ...
        for(var i=this._startRow; i< this._startRow + this._rowPerScreen && i < this._totalRow; i++)
        {
            var cameraRow = i-this._startRow;
            for(var j= this._startColumn; j< this._startColumn + this._columnPerScreen && j < this._totalColumn; j++)
            {
                ...
                if(areaType.bgKey)
                {
                    ...
                    internalContext.drawImage(img,cameraColumn * this._gridSize,cameraRow* this._gridSize,
    ...
    MapRenderer.prototype._calculateCamera = function(){
        ...
        if(this._startRow < 0)
            this._startRow = 0;
        else if(this._player.getRow() + this._idealCameraRowRange >= this._totalRow)
            this._startRow = this._totalRow - (this. _idealCameraRowRange * 2);


        this._startColumn = this._player.getColumn()- this. _idealCameraColRange ;
        if(this._startColumn <0)
            this._startColumn =0;
        else if(this._player.getColumn() + this. _idealCameraColRange >= this._totalColumn)
            this._startColumn = this._totalColumn - (this. _idealCameraColRange *2);
    ...
    MapRenderer.prototype._calculateViewport = function(){
        ...
        this._gridSize = canvas.width / this._columnPerScreen;
